...
func (s *Service) processBlock(ctx context.Context, client *blockchain.ChainClient, txProcessor *TxProcessor, blockNum int64, chainID int64) error {
	...
	// Process all transactions in the block (includes balance updates)
	if err := txProcessor.ProcessBlockTransactions(ctx, block, chainID); err != nil {
		s.logger.Warnw("Failed to process transactions", "block", blockNum, "error", err)
		// Continue - block is saved even if tx processing fails
	}
